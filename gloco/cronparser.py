...
## USA.
##
## Author(s): Adriano Monteiro Marques <py.adriano@gmail.com>
##
"""
gloco/cronparser.py

Cron-compatible date/time/frequency parser
"""
...
class CronParser:
    ...
    def parse(self, expression, start=1, end=31, zero_full=False):
        ...
        for i in result:
            if i < start or i > end:
                raise Exception("Expression '%s' out of boundaries." \
                                "Start at %s until %s" % (i, start, end))
            if i not in res:
                res.append(i)
        del(result)
...
if __name__ == '__main__':
    ...
    print "Result: MONTH", CronParser().parse_month('jan-jun')
